...
import { getCarts, deleteCartById } from 'apiSdk/carts';
import { CartInterface } from 'interfaces/cart';

type ColumnType = ColumnDef<CartInterface, unknown>;
...
export function CartListPage(props: CartListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CartInterface>>(
    () => `/carts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CartInterface) => {
    if (hasAccess('cart', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/carts/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/carts/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('cart', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/carts/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('cart', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="cart" />
        </Flex>

        {hasAccess('cart', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/carts/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
