...
import { getCategories, deleteCategoryById } from 'apiSdk/categories';
import { CategoryInterface } from 'interfaces/category';

type ColumnType = ColumnDef<CategoryInterface, unknown>;
...
export function CategoryListPage(props: CategoryListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CategoryInterface>>(
    () => `/categories?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CategoryInterface) => {
    if (hasAccess('category', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/categories/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/categories/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('category', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/categories/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('category', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="category" />
        </Flex>

        {hasAccess('category', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/categories/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
