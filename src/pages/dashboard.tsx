...
function DashboardPage() {
  ...
  const categoryStat = useSWR<PaginatedInterface<CategoryInterface>>(
    () => `/categories`,
    () => getCategories({ limit: 1, offset: 0 }),
  );

  const productStat = useSWR<PaginatedInterface<ProductInterface>>(
    () => `/products`,
    () => getProducts({ limit: 1, offset: 0 }),
  );

  const cartStat = useSWR<PaginatedInterface<CartInterface>>(
    () => `/carts`,
    () => getCarts({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Categories',
      result: categoryStat?.data?.totalCount,
      isLoading: categoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Products',
      result: productStat?.data?.totalCount,
      isLoading: productStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Carts',
      result: cartStat?.data?.totalCount,
      isLoading: cartStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
